...
## Q3：训练实战——为什么现代大模型预训练（如 LLaMA）几乎全部转向了 BF16，而抛弃了以前常用的 FP16？

<details>
<summary>点击展开查看解析</summary>

这主要是由于大语言模型深层 Transformer 的数值不稳定性造成的。

**1. FP16 的痛点：动态范围太窄，极易溢出 (Overflow)**
大模型在训练时，尤其是未经过归一化的梯度，或者 Attention 模块计算出来的 logits，非常容易产生大于 `65504` 的数值。一旦超过这个上限，FP16 就会数值溢出变成 `NaN`（Not a Number），导致几百张卡的训练瞬间崩溃。为了强行用 FP16 训练，以前必须引入繁琐且极不稳定的 **Loss Scaling (动态损失缩放)** 技术。

**2. BF16 的压倒性优势：无脑、稳定、免调参**
正如 Q2 中提到的，BF16 拥有 $10^{38}$ 的恐怖动态范围，绝对不可能发生数值溢出。
虽然 BF16 牺牲了 3 位的尾数精度，但神经网络本身是一个极其粗线条的统计学模型，它对微小的小数点精度损失**极其鲁棒**，但对“数值范围溢出”却**极其脆弱**。因此，用极小的精度代价换取绝对的训练稳定性，BF16 成为了现代大模型预训练和微调的绝对标配。
</details>
